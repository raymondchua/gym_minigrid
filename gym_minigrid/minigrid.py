...
class Goal(WorldObj):
    def __init__(self, show_goal: bool = None):
        assert show_goal is not None
        self.show_goal = show_goal
        if self.show_goal:
            super().__init__("goal", "green")
        else:
            super().__init__("goal", "black")
    ...
    def render(self, img):
        if self.show_goal:
            ...
            fill_coords(img, point_in_rect(0, 0, 0, 0), COLORS[self.color])
...
class Grid:
    ...
    @staticmethod
    def decode(array, show_goal: bool):
        """
        ...
        for i in range(width):
            for j in range(height):
                ...
                v = WorldObj.decode(type_idx, color_idx, state, show_goal=show_goal)
                grid.set(i, j, v)
...
class MiniGridEnv(gym.Env):
    ...
    def __init__(
        ...
        assert show_goal is not None, "show_goal must be set"
        self.show_goal = show_goal
    ...
    def reset_with_agent_pos_dir(self, agent_pos=None, agent_dir=None):
        # Current position and direction of the agent
        ...
        # To keep the same grid for each episode, call env.seed() with
        # the same seed before calling env.reset()
        self._gen_grid_custom(
            self.width,
            self.height,
            custom_agent_pos=agent_pos,
            custom_agent_dir=agent_dir,
        )
    ...
    def _gen_grid_custom(self, width, height, custom_agent_pos, custom_agent_dir):
        assert False, "_gen_grid_custom needs to be implemented by each environment"
    ...
    def agent_sees(self, x, y):
        ...
        obs = self.gen_obs()
        obs_grid, _ = Grid.decode(obs["image"], show_goal=self.show_goal)
    ...
    def get_obs_render(self, obs, tile_size=TILE_PIXELS // 2):
        ...
        grid, vis_mask = Grid.decode(obs, show_goal=self.show_goal)
