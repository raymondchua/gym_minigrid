...
class VertWallBottom2Env(MiniGridEnv):
    """
    Environment with wall sparse reward. The top left yellow box has a reward of -1.0 and the
    bottom right yellow box has a reward of 1.0.
    """
    ...
    def _gen_grid(self, width, height):
        ...
        if self.show_goal:
            ...
            self.put_obj(Box("yellow"), self.goal_pos_2["x"], self.goal_pos_2["y"])
    ...
    def step(self, action):
        ...
        # Move forward
            ...
            if fwd_cell is not None and fwd_cell.type == "box":
                ...
                # negative reward if it is in the top left and positive reward if it is in the bottom right
                if (
                    fwd_pos[0] == self.goal_pos_1["x"]
...
class GridworldVertWallBottom2(VertWallBottom2Env):
    def __init__(self):
        super().__init__(
            size=12,
...
class GridworldVertWallBottom2NoGoalVis(VertWallBottom2Env):
    def __init__(self):
        super().__init__(
            size=12,
            goal_pos_1=dict(x=1, y=1),
...
register(
    id="MiniGrid-Gridworld-VertWallBottom2-v0",
    entry_point="CRLMSF.environments.gym_minigrid.envs:GridworldVertWallBottom2",
)

register(
    id="MiniGrid-Gridworld-VertWallBottom2-NoGoalVis-v0",
    entry_point="CRLMSF.environments.gym_minigrid.envs:GridworldVertWallBottom2NoGoalVis",
)
